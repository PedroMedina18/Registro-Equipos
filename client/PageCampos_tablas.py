...
from config import FONT_LABEL,FONT_LABEL_TITULO, COLOR_BASE, COLOR_AZUL,TITULO_CAMPOS, COLOR_ROJO, COLOR_VERDE, ACTIVE_VERDE, ACTIVE_AZUL, ACTIVE_ROJO
    ...
    def guardar_campos(self):
        try:
            ...
            if(self.id_campos_tabla==None):
                Campos_Tabla.create(nombre=tipo_equipo["nombre"], descripcion=tipo_equipo["descripcion"], caracteres=tipo_equipo["caracteres"])
            else:
                Campos_Tabla.update(id=self.id_campos_tabla, nombre=tipo_equipo["nombre"], descripcion=tipo_equipo["descripcion"], caracteres=tipo_equipo["caracteres"])
        
        except:
            messagebox.showwarning(TITULO_CAMPOS, f'Campo Caracteres. Solo se permiten numeros')
            return None
        finally:
            self.desabilitar_campos()
            self.tabla_lista()
